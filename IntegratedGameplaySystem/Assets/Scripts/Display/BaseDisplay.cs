...
namespace IntegratedGameplaySystem
{
    ...
    public class BaseDisplay : IDisposable
    {
        ...
        public static void RangeIntoText(IntWithMax range, Text text) => text.text = $"({range.min} / {range.max})";
        public static void FloatIntoText(float value, Text text) => text.text = value.ToString();
        public static void RangeIntoFillImage(IntWithMax range, Image fillImg) => fillImg.fillAmount = (float)range.min / range.max;
